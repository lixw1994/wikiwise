...
- `npm --workspace @wikiwise/electron-app test -- test/native-shell-parity.test.js` exited 1 before implementation, with the new test failing because `.welcome-panel .summary` still used `line-height: 1.55` instead of the native-derived `19px`.

### GREEN

- `npm --workspace @wikiwise/electron-app test -- test/native-shell-parity.test.js` exited 0 after implementation, with 16/16 tests passing.

### Full Checks

- `npm test` exited 0, with 214/214 Electron tests and 31/31 core tests passing.

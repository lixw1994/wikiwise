...
test("main process repairs node-pty spawn helper permissions before PTY startup", () => {
  ...
  const startTerminalStart = mainSource.indexOf("function startTerminal");
  const normalizeStart = mainSource.indexOf("function normalizeTerminalSize", startTerminalStart);
  const resolveHelperStart = mainSource.indexOf("function resolveNodePtySpawnHelperPaths");
  ...
  assert.match(mainSource, /fs\.readdirSync\(prebuildsPath,\s*\{\s*withFileTypes:\s*true\s*\}\)/);
  assert.match(mainSource, /entry\.isDirectory\(\)/);
  assert.match(mainSource, /entry\.name\.startsWith\("darwin-"\)/);
  assert.match(mainSource, /path\.join\(prebuildsPath,\s*entry\.name,\s*"spawn-helper"\)/);
  assert.match(ensureHelperSource, /process\.platform === "win32"/);
  assert.match(ensureHelperSource, /const helperPaths = resolveNodePtySpawnHelperPaths\(\)/);
  assert.match(ensureHelperSource, /for \(const helperPath of helperPaths\)/);
  ...
  assert.doesNotMatch(mainSource, /prebuilds`,?\s*`\$\{process\.platform\}-\$\{process\.arch\}`/);
